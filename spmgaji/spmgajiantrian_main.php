<?php
function sppgajiantrian_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	$output_form = drupal_get_form('sppgajiantrian_main_form');
	$header = array (
	...
	if ($kodeuk !='##') $query->condition('g.kodeuk', $kodeuk, '=');
	if ($bulan !='0') $query->condition('g.bulan', $bulan, '=');
	
	if ($proses !='##') $query->condition('g.proses', $proses, '=');
	...
	foreach ($results as $data) {
		...
		if($data->proses=='1'){
			...
			$editlink = apbd_button_jurnalkan('sppgaji/new/' . $data->nourut);
		}
...
function sppgajiantrian_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$uri = 'sppgajiantrian/filter/' . $kodeuk . '/' . $bulan . '/' . $proses ;
	drupal_goto($uri);
	
}


function sppgajiantrian_main_form($form, &$form_state) {
	...
	$opt_jurnal['##'] ='SEMUA';
	$opt_jurnal['0'] = 'ANTRIAN';
	$opt_jurnal['1'] = 'POSTING';	

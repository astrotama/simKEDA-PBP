...
function spmgaji_edit_main_form($form, &$form_state) {
	...
	$spmtgl = mktime(0,0,0,12,16,2017);
	$bulan = date('m');
	$spdno = '';
	$spdtgl = '';
	$keperluan = 'Gaji Januari 2017';
	$jeniskegiatan = '1';
	$penerimanama = '';
	$penerimapimpinan = '';
	$penerimaalamat = '';
	...
	foreach ($results as $data) {
		...
		$title = 'SPM Gaji Bulan ' . $data->bulan ;
		
		$dokid = $data->dokid;
		$spmno = $data->spmno;
		if (is_null($data->spmtgl))
			$spmtgl = mktime(0,0,0,date('m'),date('d'),2017);
		else
...
function printspm($dokid) {
	...
	$rows[]=array(
		array('data' => 'TAHUN ANGGARAN 2017', 'width' => '510px','align'=>'center','style'=>'border:none;font-size:130%;'),
	);
	$rows[]=array(
